...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@gametheory/sdk'
...
describe('utils', () => {
  describe('#getSnowTraceLink', () => {
    it('correct for tx', () => {
      expect(getSnowTraceLink(1, 'abc', 'transaction')).toEqual('https://snowtrace.io/tx/abc')
    })
    it('correct for token', () => {
      expect(getSnowTraceLink(1, 'abc', 'token')).toEqual('https://snowtrace.io/token/abc')
    })
    it('correct for address', () => {
      expect(getSnowTraceLink(1, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getSnowTraceLink(2, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('ropsten', () => {
      expect(getSnowTraceLink(3, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('enum', () => {
      expect(getSnowTraceLink(ChainId.MAINNET, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
  })
