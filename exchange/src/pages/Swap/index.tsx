...
const Swap = () => {
  ...
  const rawInput = parsedAmounts[Field.INPUT]?.raw.toString() ?? "0";
  const [sellTax, amountInWithTax, extraSlippage] = useSellTax(currencies[Field.INPUT], ethers.utils.formatEther(rawInput), trade?.route, trade?.inputAmount?.raw?.toString());
  const slippage = allowedSlippage + extraSlippage;//+ Math.ceil(parseFloat(ethers.utils.formatUnits(sellTax, 2)) * 100);
  //if(trade) trade.inputAmount.numerator = JSBI.BigInt(amountInWithTax.toString());
  ...
  const [approval, approveCallback] = useApproveCallbackFromTrade(trade, slippage)
            ...
            <BottomGrouping>
              {!account ? (
                ...
                <GreyCard style={{ textAlign: 'center' }}>
                  <Text mb="4px">{TranslateString(1194, 'No route found or route is still loading.')}</Text>
                </GreyCard>
              ) : showApproveFlow ? (
                <RowBetween>
                  <Button
