    ...
    DECIMAL_PERMUTATIONS.forEach(decimals => {
        describe(`decimals permutation: ${decimals}`, () => {
            ...
            it('Price:Route.midPrice', () => {
                ...
                expect(route.midPrice.quote(new TokenAmount(route.input, decimalize(1, route.input.decimals)))).toEqual(new TokenAmount(route.output, decimalize(1234, route.output.decimals)));
                expect(route.midPrice.invert().quote(new TokenAmount(route.output, decimalize(1234, route.output.decimals)))).toEqual(new TokenAmount(route.input, decimalize(1, route.input.decimals)));
                ...
                it('minimum TradeType.EXACT_INPUT', () => {
                    if ([9, 18].includes(tokens[1].decimals)) {
                        const route = new Route([
                            new Pair(new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals)), new TokenAmount(WETH, decimalize(10, WETH.decimals) +
