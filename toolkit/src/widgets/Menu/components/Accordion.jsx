...
var Accordion = function (_a) {
    ...
    return (<Container>
      <MenuEntry_1.MenuEntry onClick={handleClick} className={className} isActive={isActive} role="button">
        {icon}
        <MenuEntry_1.LinkLabel isPushed={isPushed}>{label}</MenuEntry_1.LinkLabel>
        {status && (<MenuEntry_1.LinkStatus color={status.color} fontSize="14px">
            {status.text}
          </MenuEntry_1.LinkStatus>)}
        {isOpen ? <Svg_1.ArrowDropUpIcon /> : <Svg_1.ArrowDropDownIcon />}
      </MenuEntry_1.MenuEntry>
      <AccordionContent isOpen={isOpen} isPushed={isPushed} maxHeight={react_1.default.Children.count(children) * config_1.MENU_ENTRY_HEIGHT}>
        {children}
      </AccordionContent>
    </Container>);
};
