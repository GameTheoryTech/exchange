...
async function connect() {
  ...
  // set provider
  provider = window.ethereum || window.web3.currentProvider || await web3Modal.connect();
  web3 = new Web3(provider);

  providerServer = new Web3.providers.HttpProvider(web3ServerRpcUrl);
  web3Server = new Web3(providerServer);

  // set current network id
  web3gl.networkId = parseInt(await window.ethereum.request({ method: 'eth_chainId' }) || provider.chainId);
  ...
  web3gl.connectAccount = (await window.ethereum.enable())[0] || provider.accounts[0];
...
async function sendTransaction(to, value, gasLimit, gasPrice) {
  ...
  web3.eth
    ...
    .on("receipt", (receipt) => {
      window.web3gl.sendTransactionResponse = txHash || receipt.transactionHash;
    })
    .on("error", (error) => {
...
async function sendContract(method, abi, contract, args, value, gasLimit, gasPrice) {
  ...
  new web3.eth.Contract(JSON.parse(abi), contract).methods[method](...JSON.parse(args))
    ...
    .on("transactionHash", (transactionHash) => {
      txHash = transactionHash;
      waitForReceipt(txHash, (receipt) => window.web3gl.sendContractResponse = txHash || receipt.transactionHash); //"receipt" event not working anymore
    })
    // .on("receipt", (receipt) => {
    //   window.web3gl.sendContractResponse = txHash || receipt.transactionHash;
    // })
    .on("error", (error) => {
...
async function callContract(method, abi, contract, args, value, gasLimit, gasPrice) {
  const from = (await web3.eth.getAccounts())[0];
  new web3.eth.Contract(JSON.parse(abi), contract).methods[method](...JSON.parse(args))
      .call({
        from,
        ...
        {
          if(result.hasOwnProperty("toString")) window.web3gl.sendContractResponse = `Success: ${result.toString()}`;
          else window.web3gl.sendContractResponse = `Success: ${result}`;
        }
        else result = "Success: "
      });
}

async function callContractServer(method, abi, contract, args, value, gasLimit, gasPrice) {
    const from = (await web3.eth.getAccounts())[0];
    new web3Server.eth.Contract(JSON.parse(abi), contract).methods[method](...JSON.parse(args))
        .call({
            from,
            value,
            gas: gasLimit ? gasLimit : undefined,
